...
# Isko global define kiya hai taaki function objects access kar sakein
LOOKUPS_CONFIG = {
    '📱 NUMBER LOOKUP': {
    ...
    '📸 INSTAGRAM LOOKUP': {
        ...
        'error_msg': "❌ Invalid Instagram username!"
    },
    '📞 TELEGRAM LOOKUP': {
...
╚══════════════════════════════════╝"""
    ...
    # Handle Button Press
    if text in LOOKUPS_CONFIG:
        cfg = LOOKUPS_CONFIG[text]
        send_message(chat_id, cfg['prompt'], get_cancel_keyboard())
        # Save only simple data types (strings), not functions
        state['stage'] = cfg['stage']
        state['action'] = text  # Save the button text to retrieve config later
        save_state(chat_id, state)
        return
    
    # Handle User Input (when waiting for data)
    if state.get('stage', '').startswith('awaiting_'):
        action_key = state.get('action')
        
        # Retrieve config using the saved action key
        if action_key and action_key in LOOKUPS_CONFIG:
            cfg = LOOKUPS_CONFIG[action_key]
            
            query = text
            if cfg.get('clean') == 'upper':
                query = query.upper()
            
            # Validation
            if 'pattern' in cfg:
                if not re.match(cfg['pattern'], query):
                    send_message(chat_id, cfg.get('error_msg', "❌ Invalid input!"), get_cancel_keyboard())
                    return
            
            # Process Request
            send_message(chat_id, "⏳ Fetching details...")
            url = cfg['api_endpoint'].replace('{term}', query)
            response = http_get(url)
            
            # Use the function from config
            ...
            # Reset state
            state['stage'] = 'idle'
            if 'action' in state: del state['action']
            save_state(chat_id, state)
            return
